  ...
  return (
    <Section variant="default" container="wide" containerClassName="max-w-5xl">
      ...
      <div className="bg-accent-teal/5 border-accent-teal/15 mt-8 rounded-2xl border p-6">
        <h2 className="text-lg font-semibold text-ink">Quick answer</h2>
        <p className="mt-2 leading-8 text-muted-foreground">{answer}</p>
      </div>
      ...
      <ul className="mt-6 grid gap-3 md:grid-cols-2">
        {bullets.map((bullet) => (
          <li
            key={bullet}
            className="shadow-soft flex items-start gap-3 rounded-2xl border border-border bg-white p-4 leading-7 text-muted-foreground"
          >
      ...
      <div className="shadow-soft mt-12 overflow-x-auto rounded-2xl border border-border bg-white">
        <table className="w-full text-left text-sm">
          <thead className="text-ink bg-muted">
            <tr>
